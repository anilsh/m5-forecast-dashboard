...
def load_data():
    ...
    #train_df      = pd.read_csv(fp("train_df.csv"), parse_dates=["date"])
    agg_sales     = pd.read_csv(fp("agg_sales.csv"))
    cat_sales     = pd.read_csv(fp("cat_sales.csv"))
...
def apply_theme(fig, title="", height=380):
    fig.update_layout(
        title=dict(text=title, font=dict(family="Space Mono", size=13, color="#1a1d2e")),
        paper_bgcolor="white",
        plot_bgcolor="#f7f8fc",
...
metrics = compute_metrics(val_df, val_df)
...
if section == "🏠  Overview":
    ...
    kpi(k1, "RMSSE", f"{0.7978:.4f}", "< 1.0 beats naïve baseline ✓", "#4f6ef7")
...
# ═══════════════════════════════════════════════════════════════
    ...
    with r1c2:
        ...
        fig2.add_vline(x=0, line_dash="dash", line_color="#1a1d2e",
...
# ═══════════════════════════════════════════════════════════════
    ...
    if sample.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color="#1a1d2e", width=2),
            marker=dict(size=5, color="#1a1d2e"),
        ))
        fig.add_trace(go.Scatter(
        ...
        fig_err.add_hline(y=0, line_color="#1a1d2e", line_width=1)
